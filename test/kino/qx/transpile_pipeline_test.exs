defmodule Kino.Qx.TranspilePipelineTest do
  ...
  describe "run/1 happy path" do
    test "sequences all stages and returns merged result", ctx do
      ...
      # Verify call ORDER (key sequence, not args). No sessions: no
      # open_session / close_session in the sequence.
    ...
    test "submit receives the transpiled qasm + default shots, not the original", ctx do
      script_happy_path(ctx.recorder)
    ...
    test "shots threads through to submit_sampler", ctx do
      script_happy_path(ctx.recorder)
      assert {:ok, _} = TranspilePipeline.run(base_input(ctx, %{shots: 1024}))
  ...
  describe "error routing" do
    ...
    test "job ends with status Failed → :ibm_job_failed", ctx do
      script_happy_path(
        ctx.recorder,
        polls: [
          {:ok, %{status: "Queued", reason: nil}},
  ...
  describe "on_status callback" do
    ...
    test ":job_started carries the job_id (used by cell's cancel flow)", ctx do
      script_happy_path(ctx.recorder)
